...
public class PaymentProcessService {
    ...
    private void processPaymentWithRetry(PaymentValidationEvent event) {
        log.info("[Payment Process Service][Async] Passing payment for Seat Id={} to Bank Service for further processing. Payment Id={}",
        ...
        try {
            ...
            log.error("[Payment Process Service][Retry] Payment Id={} failed after retries due to {}", event.getPayment().getId(), e.getMessage());
    ...
    private void processBankResponse(Payment payment, BankPaymentResponse response) {
        log.info("[Payment Process Service] Bank call completed for Payment Id={} with Status={}.", payment.getId(), payment.getStatus());
        payment.setStatus(
